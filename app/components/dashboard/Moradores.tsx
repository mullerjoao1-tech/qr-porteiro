...
export default function Moradores({
  ...
  const [arquivandoMoradorId, setArquivandoMoradorId] =
  ...
  const moradoresFiltrados = useMemo(() => {
    ...
    return moradores
      .filter((morador) => {
        ...
        const passaStatus =
          statusFiltro === "todos"
            ? morador.status !== "arquivado"
            : morador.status === statusFiltro;
  ...
  async function confirmarArquivamentoMorador(
    morador: Morador
  ) {
    if (
      arquivandoMoradorId ||
    ...
    const confirmou =
      window.confirm(
        `Remover "${morador.nome}" da lista ativa deste local?

O hist?rico e a identidade universal ser?o preservados.`
      );
    ...
    try {
      setArquivandoMoradorId(
        morador.id
      );

      await arquivarMorador(
        morador.id
      );
    } catch (erro) {
      console.error(
        "Erro ao arquivar morador:",
        erro
      );
      ...
      alert(
        erro instanceof Error
          ? erro.message
          : "N?o foi poss?vel arquivar o morador."
      );
    } finally {
      setArquivandoMoradorId(
        null
                ...
                <div
                  ...
                  onKeyDown={(event) => {
                    if (event.key === "Enter" || event.key === " ") {
                      abrirMorador(morador);
                    }
                  }}
                  className="cursor-pointer rounded-2xl border border-slate-700 bg-slate-800 p-4 text-left transition-all hover:border-blue-500 hover:bg-slate-700 active:scale-[0.98]"
                >
                      ...
                      {arquivarMorador && (
                        <button
                          ...
                          onClick={(event) => {
                            ...
                            void confirmarArquivamentoMorador(
                              morador
                            );
                          }}
                          className="mb-3 ml-auto flex h-9 w-9 items-center justify-center rounded-xl border border-red-900 bg-red-950/30 text-base text-red-300 transition hover:bg-red-950/60 disabled:opacity-50"
                        >

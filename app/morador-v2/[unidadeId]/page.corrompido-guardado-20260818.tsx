...
const nomeLocal =
  ...
  useEffect(() => {
    ...
    const pararDeOuvir = onValue(referencia, async (snapshot) => {
      ...
      /*
       * Compatibilidade:
       * chamadas antigas sem responsÃ¡vel individual
       * continuam funcionando como antes.
       *
       * Chamadas novas com responsÃ¡vel definido
       * pertencem somente ao usuÃ¡rio selecionado
...
async function naoPossoAtender() {
  ...
  try {
    ...
    if (
      ...
      setAvisoAuto(
        `âœ“ Chamada encaminhada para ${resultado.responsavel.nome}.`
      );
    ...
    setAvisoAuto(
      resultado.motivo ||
      "Nenhum responsÃ¡vel disponÃ­vel. Chamada encerrada."
    );
  ...
  async function acionarPortao() {
    ...
    try {
      ...
      await registrarLog("portao_tentativa", "Tentativa de abertura do portÃ£o");

      const resposta = await fetch("/api/abrir-portao");
      const dados = await resposta.json();

      if (dados.success) {
        setStatusPortao("âœ… PortÃ£o aberto com sucesso");
        await registrarLog("portao_sucesso", "PortÃ£o aberto com sucesso");
      } else {
        setStatusPortao("âŒ Falha ao abrir portÃ£o");
        await registrarLog(
          "erro_portao",
          "API respondeu falha ao abrir portÃ£o"
        );
      }
    } catch (erro) {
      setStatusPortao("âŒ Erro ao abrir portÃ£o");

      await registrarLog("erro_portao", "Erro inesperado: " + String(erro));

      console.error("Erro ao abrir portÃ£o:", erro);
    } finally {
      setTimeout(() => {
  ...
  async function ativarNotificacoes() {
    try {
      if (typeof window === "undefined") {
        alert("As notificaÃ§Ãµes sÃ³ podem ser ativadas no navegador.");
        return;
      }
      ...
      if (!("Notification" in window)) {
        await registrarLog(
          "push_nao_suportado",
          "Este navegador nÃ£o possui suporte Ã  API de notificaÃ§Ãµes"
        );
      ...
      if (!("serviceWorker" in navigator)) {
        await registrarLog(
          ...
          "Este navegador nÃ£o possui suporte a Service Worker"
        );

        alert("Este navegador nÃ£o suporta notificaÃ§Ãµes em segundo plano.");
        return;
      }
      ...
      if (!messaging) {
        await registrarLog(
          "push_nao_suportado",
          "Firebase Messaging nÃ£o foi inicializado neste navegador"
        );

        alert("NÃ£o foi possÃ­vel iniciar as notificaÃ§Ãµes neste navegador.");
        return;
      }
      ...
      if (permissao !== "granted") {
        await registrarLog(
          ...
          "PermissÃ£o para notificaÃ§Ãµes negada: " + permissao
        );
  ...
  serviceWorkerRegistration: registroServiceWorker,
      ...
      if (!token) {
        throw new Error(
          "O Firebase nÃ£o retornou um token de notificaÃ§Ã£o."
        );
      }
      const condominioId =
      ...
      await registrarLog(
        "push_token_salvo",
        "Token de notificaÃ§Ã£o salvo com sucesso"
      );

      alert("NotificaÃ§Ãµes ativadas com sucesso!");
    } catch (erro: any) {
      console.error("Erro completo ao ativar notificaÃ§Ãµes:", erro);
      ...
      await registrarLog(
        "push_erro_token",
        `CÃ³digo: ${codigo} | Mensagem: ${mensagemErro}`
      );
...
Mensagem: ${mensagemErro}`
  ...
  function abrirAudioVisitanteGrande() {
    ...
    setAudioPopup({
      titulo: "ðŸŽ™ï¸ Ãudio do visitante",
  ...
  const respostasRapidas = [
    ...
    {
      texto: "JÃ¡ estou descendo",
      mensagem: "OlÃ¡, entendi. JÃ¡ estou descendo.",
      icone: "ðŸš¶",
    },
    {
    ...
    {
      texto: "NÃ£o estou em casa",
      mensagem: "NÃ£o estou em casa no momento.",
      icone: "ðŸ ",
    },
    {
                  ...
                  comunicadoAberto.visualizacoes?.[slug]?.ciente === true ? (
                    ...
                    >
                      {salvandoCiencia
                        ? "Registrando..."
                        : ehEmergencia
                        ? "ðŸš¨ Confirmo que li e estou ciente"
                        : "âœ… Li e estou ciente"}
                    </button>
                  )
                ) : (
                  <button
      ...
      {mostrarPopupChamada && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4 overflow-y-auto">
          <div className="w-full max-w-md bg-slate-900 border-4 border-green-400 rounded-3xl p-5 text-center shadow-2xl my-4">
            <p className="text-6xl mb-3">ðŸš¨</p>
...
>
  ðŸ”µ NÃƒO POSSO ATENDER
</button>
            <div className="mt-4 bg-slate-800 border border-yellow-500/40 rounded-2xl p-3">
              <p className="text-yellow-300 text-sm font-bold">
                OuÃ§a o Ã¡udio e veja a cÃ¢mera. Para responder, clique em ATENDER AGORA.
              </p>
            </div>
          ...
          <div className="relative w-full max-w-md bg-slate-900 border-2 border-cyan-500 rounded-3xl p-5 shadow-2xl">
            ...
            <div className="text-center mb-5">
              <div className="text-5xl mb-3">
                {gravandoAudioMorador ? "ðŸŽ™ï¸" : "ðŸŽ§"}
              </div>
      ...
      {audioPopup && (
        ...
        <div className="fixed inset-0 z-[999] bg-black/95 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-slate-900 border-4 border-blue-400 rounded-3xl p-5 text-center shadow-2xl">
            <p className="text-6xl mb-3">ðŸŽ™ï¸</p>
        ...
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4 overflow-y-auto">
          <div className="w-full max-w-md bg-slate-900 border-4 border-green-400 rounded-3xl p-5 text-center shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              ...
              >
                {capturandoCamera ? "ðŸ“¸ Atualizando" : "ðŸ“¸ Atualizar"}
              </button>
        ...
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4 overflow-y-auto">
          <div className="w-full max-w-md bg-slate-900 border-4 border-slate-600 rounded-3xl p-5 shadow-2xl my-4">
            <div className="flex items-center justify-between gap-3 mb-4">
              <h3 className="text-2xl font-black">ðŸ“‹ HistÃ³rico</h3>
            ...
            {historicoLista.length > 0 ? (
                ...
                {historicoLista.map((item, index) => (
                  ...
                  >
                    ...
                    <p className="text-blue-300 text-xs mt-1">
                      Tipo: {item.tipoFinalizacao || "NÃ£o informado"}
                    </p>
                  </div>
                ))}
              </div>
              ...
              <p className="text-green-400 text-sm bg-slate-800 rounded-2xl p-4">
                ðŸ”” Nenhum atendimento finalizado
              </p>
            )}
          </div>
        </div>
      ...
      <div className="w-full max-w-md mx-auto bg-slate-900 rounded-3xl p-5 shadow-2xl border border-slate-800">
        <div className="flex items-start justify-between gap-3">
          ...
          >
            {online ? "ðŸŸ¢ DisponÃ­vel" : "ðŸ”´ Ausente"}
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          <button
            ...
            onClick={() => {
              if (!cameraContratada) {
                alert(
                  "ðŸ“· Recurso opcional\n\nA cÃ¢mera Ã© um recurso opcional do QR Acesso. Consulte a administraÃ§Ã£o para ativaÃ§Ã£o."
                );
                return;
              }
          ...
          <button
            ...
            onClick={() => {
              if (!portaoContratado) {
                alert(
                  "ðŸšª Recurso opcional\n\nA abertura de portÃ£o Ã© um recurso opcional do QR Acesso. Consulte a administraÃ§Ã£o para ativaÃ§Ã£o."
                );
                return;
              }
        ...
        <div className="bg-slate-800 rounded-2xl p-4 mt-4 border border-green-500/20">
          <h2 className="font-black text-green-400 text-xl">ðŸ”” {nome}</h2>
                    ...
                    {mensagensConversa.map((item) => (
                      ...
                      >
                        <p className="text-xs font-black mb-2">
                          {item.autor === "morador" ? "VocÃª" : "Visitante"}
                        </p>

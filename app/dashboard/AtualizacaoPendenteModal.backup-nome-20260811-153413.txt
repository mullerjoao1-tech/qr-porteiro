    ...
    try {
      ...
      if (moradoresDados) {
        ...
        moradorExistente =
          listaMoradores.find(
            (morador) =>
              morador.unidadeId === atualizacao.unidadeId &&
      ...
      if (moradorExistente) {
        ...
        await update(
          ...
          {
            nome: formatarNome(atualizacao.nome),
            telefone: atualizacao.telefone.trim(),
            email: atualizacao.email?.trim() || "",
            unidadeId: atualizacao.unidadeId,
            unidadeNome: `${atualizacao.condominioNome} • ${atualizacao.unidadeNome}`,
            localId: atualizacao.condominioId,
            localNome: atualizacao.condominioNome,
      ...
      <div className="w-full max-w-3xl max-h-[92vh] overflow-y-auto rounded-3xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          <button
            type="button"
            onClick={() =>
              acaoAindaNaoImplementada("Editar antes de aprovar")
            }
            disabled={aprovando}

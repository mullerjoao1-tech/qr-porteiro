...
export default function Painel() {
  ...
  const caminhoFirebase = "qr2";
  const caminhoHistorico = "historico/qr2";
  const caminhoStatus = "status/qr2";
...
const TEMPO_EM_ATENDIMENTO = 3 * 60 * 1000;
  ...
  async function registrarLog(tipo: string, detalhes: string) {
    try {
      const novoLog = push(ref(db, "logs/qr2"));
  ...
  async function registrarAnalytics(evento: string) {
    try {
      const referencia = ref(db, "analytics/qr2");
  ...
  useEffect(() => {
    const referenciaAnalytics = ref(db, "analytics/qr2");
  ...
  async function zerarMetricas() {
    ...
    await set(ref(db, "analytics/qr2"), {
      recebidas: 0,
      atendidas: 0,
      finalizadas: 0,
      timeouts: 0,
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-950 text-white p-4 relative">
      ...
      <div className="w-full max-w-md bg-slate-900 rounded-2xl p-8">
        <h1 className="text-4xl font-bold mb-2">🏠 Painel do Morador 2</h1>
        ...
        <div className="bg-slate-800 rounded-xl p-4 mb-4 border border-blue-500/30">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-bold text-blue-300">📊 Métricas do qr2</h2>
